...
@property (strong, nonatomic) ZCAssetsLibrary *library;
@property (strong, nonatomic) dispatch_queue_t videoQueue;

@end

@implementation ZCBaseCameraController

- (instancetype)init {
	self = [super init];
	if (self) {
		_library = [[ZCAssetsLibrary alloc] init];
	}
	return self;
}
    ...
    if (videoInput) {
        if ([self.captureSession canAddInput:videoInput]) {
			...
			*error = [NSError errorWithDomain:ZCCameraErrorDomain
										 code:ZCCameraErrorFailedToAddInput
									 userInfo:userInfo];
			return NO;
		}
    } else {
...
- (NSURL *)uniqueURL {
    ...
    NSString *dirPath =
	[fileManager temporaryDirectoryWithTemplateString:@"cusen.XXXXXX"];

    if (dirPath) {
        NSString *filePath =
		[dirPath stringByAppendingPathComponent:@"cusen_movie.mov"];
...
didFinishRecordingToOutputFileAtURL:(NSURL *)outputFileURL
	...
	if (error) {
		...
		__weak ZCBaseCameraController *weakSelf = self;
