...
// rgb
#define ZCColor(r, g, b) [UIColor colorWithRed:(r) / 255.0f green:(g) / 255.0f blue:(b) / 255.0f alpha:1.0]
...
- (void)setSliderHeight:(CGFloat)sliderHeight {
    ...
    if (self.sliderView) {
        ...
        frame.origin.y = self.frame.size.height - sliderHeight - 0.5;
...
- (void)setBorderRadius:(CGFloat)radius {
    ...
    ViewBorderRadius(self, radius, self.edgingWidth, self.edgingColor);
    if (self.sliderView) {
        ViewBorderRadius(self.sliderView, self.frame.size.height / 2, 0, [UIColor clearColor]);
    }
}
...
- (void)setupSliderView {
    self.sliderHeight = 1.0f;
    UIView *sliderView = [[UIView alloc] initWithFrame:(CGRect){0, self.frame.size.height-self.sliderHeight-0.5, self.frame.size.width / self.titles.count, self.sliderHeight}];
...
- (void)setupBase {
    ...
    [self setupAllButton];
    // 先调一下这个方法，默认选择第一个按钮
    [self buttonClick:[self viewWithTag:ZCButtonTag]];
...
- (void)buttonClick:(UIButton *)button {
    ...
    [UIView animateWithDuration:0.25 animations:^{
        [self setContentOffset:(CGPoint){(button.tag - ZCButtonTag) * (self.frame.size.width) / self.titles.count, 0}];
    }];
    
    NSInteger selectIndex = button.tag - ZCButtonTag;
    // 调用代理和block
    if (self.titleClick) {
...
- (void)setAllColors {
    ...
    for (NSInteger i = 0; i < self.titles.count; i++) {
        UIButton *button = [self viewWithTag:i + ZCButtonTag];
        [button setTitleColor:self.normalColor forState:UIControlStateNormal];
    }
...
- (void)setContentOffset:(CGPoint)contentOffset {
    ...
    for (UIButton *button in self.allButtons) {
        CGFloat overLapWidth = CGRectIntersection(button.frame, self.sliderView.frame).size.width;
        if (overLapWidth > 0) {
    ...
    if (buttonArr.count > 1) {
        ...
        // 设置要渐变的两个button颜色
        [rightButton setTitleColor:ZCColor([self getRGBValueWithIndex:0 button:rightButton], [self getRGBValueWithIndex:1 button:rightButton], [self getRGBValueWithIndex:2 button:rightButton]) forState:UIControlStateNormal];
        [leftButton setTitleColor:ZCColor([self getRGBValueWithIndex:0 button:leftButton], [self getRGBValueWithIndex:1 button:leftButton], [self getRGBValueWithIndex:2 button:leftButton]) forState:UIControlStateNormal];
    }
    
    // 重新设置选中的button
    _selectButton = [self viewWithTag:(NSInteger)(ZCButtonTag + self.sliderView.center.x / (self.frame.size.width / self.titles.count))];
}
